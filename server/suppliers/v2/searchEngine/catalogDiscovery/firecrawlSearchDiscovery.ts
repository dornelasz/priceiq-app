/**
 * firecrawlSearchDiscovery — descobre URLs de produto via Firecrawl Search v2
 * (Etapa 15).
 *
 * Usa o endpoint /search da API Firecrawl para pesquisar
 * `"{query} site:{supplierDomain}"` e devolver APENAS URLs candidatas +
 * metadados (título/descrição). NÃO decide preço, NÃO usa LLM/schema do
 * Firecrawl, NÃO usa IA.
 *
 * Garantias (iguais ao FirecrawlProvider da Etapa 11):
 *   - A FIRECRAWL_API_KEY vai SOMENTE no header Authorization — nunca em
 *     logs/resultado.
 *   - NUNCA lança: toda falha vira um SourceFetchStatus controlado
 *     (blocked / rate_limited / no_credits / timeout / error / disabled).
 */
...
export interface FirecrawlSearchOutcome {
...
export interface FirecrawlSearchClient {
  search(
    query: string,
    opts?: { limit?: number },
  ): Promise<FirecrawlSearchOutcome>;
}

export interface FirecrawlSearchClientOptions {
  apiKey: string;
  apiUrl?: string;
  timeoutMs?: number;
  enabled?: boolean;
...
/** Monta o corpo do POST /search (exposto para teste). */
export function buildFirecrawlSearchPayload(
  query: string,
  limit: number,
...
/** Extrai itens da resposta v2 (data.web) ou v1 (data array). */
export function parseFirecrawlSearchItems(body: unknown): FirecrawlSearchItem[] {
  const b = body as { data?: { web?: unknown[] } | unknown[] } | null;
  const raw: unknown[] = Array.isArray((b?.data as { web?: unknown[] })?.web)
    ? ((b!.data as { web: unknown[] }).web)
...
export function createFirecrawlSearchClient(
  options: FirecrawlSearchClientOptions,
  ...
  return {
    async search(query, opts): Promise<FirecrawlSearchOutcome> {
      const t0 = Date.now();
      ...
      try {
        const res = await doFetch(`${apiUrl}/search`, {
          method: 'POST',
          signal: ctrl.signal,
          headers: {
          ...
          body: JSON.stringify(buildFirecrawlSearchPayload(query, limit)),
        });
        clearTimeout(timer);
        ...
        return { status: 'success', items: parseFirecrawlSearchItems(body), elapsedMs: elapsed() };
      } catch (e) {
        clearTimeout(timer);
...
export interface DiscoverViaFirecrawlSearchInput {
...
/**
 * Executa a descoberta via Firecrawl Search e devolve um SourceDiscoveryResult.
 * Monta a query com o qualificador `site:` para restringir ao domínio.
 */
export async function discoverViaFirecrawlSearch(
  input: DiscoverViaFirecrawlSearchInput,
  client: FirecrawlSearchClient,
): Promise<SourceDiscoveryResult> {
  const domain = getSupplierDomain(input.supplier);
  const searchQuery = `${input.query} site:${domain}`;
  const outcome = await client.search(searchQuery, { limit: input.maxItems });
  ...
  const attempts = [
    makeSearchAttempt({
      ...
      detail: `search "${searchQuery}" → ${urls.length} URL(s) (${outcome.status})`,
